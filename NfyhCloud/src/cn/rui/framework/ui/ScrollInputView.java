...
public class ScrollInputView extends RelativeLayout implements View.OnClickListener {
	...
	private View getWheelView(DialogViewModel m, int index) {
		// 生成一个View
		View root = LayoutInflater.from(mContext).inflate(R.layout.view_wheelview_item, null);
		WheelView view = (WheelView) root.findViewById(R.id.wheel);
		WheelViewAdapter adapter = new WheelViewAdapter(this.mContext, m.getDatas());
	 ...
	 * 
	 */
	private class WheelViewAdapter extends AbstractWheelTextAdapter implements OnWheelChangedListener, OnWheelScrollListener {
		...
		public CharSequence getItemText(int arg0) {
			if (list != null && list.size() > 0) { return list.get(arg0); }
			return null;
		}
	...
	public void setValueChangListener(WheelViewValueChangeListener valueChangListener) {
	...
	private String select() {
		...
		if (dataType == DialogViewModel.TYPE_FLOAT && (newVals != null || newVals.length > 0)) {
			try {
				// 浮点类型，赋值为第一个为新的值
			...
			valueChangListener.onValueChange(index, oldVals[index], newVals[index]);
	...
	public void onClick(View v) {
		switch (v.getId()) {
			case android.R.id.primary:
				this.valueChangListener.onValueFinsh(dataIndex, oldVals[0], select());
				break;
	...
	private void showInputDialog() {
		...
		new RuiDialog.Builder(mContext).buildTitle(name).buildMessage("请输入" + name).buildEditText("请输入" + name).buildLeftButton("返回", null).buildRight("确定", new DialogInterface.OnClickListener() {
			
			@Override
			public void onClick(DialogInterface dialog, int which) {
				RuiDialog d = (RuiDialog) dialog;
				valueChangListener.onValueFinsh(index, d.getEditText(), d.getEditText());
				dialog.dismiss();
			}
		}).show();
	}
