 ...
 * 
 */
public class AlarmRingActivity extends BaseActivity implements OnClickListener,
		SignCoreListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		View alarmView = getLayoutInflater().inflate(R.layout.activity_alarm,
				null);
		mRingTipsTextView = (TextView) alarmView
				.findViewById(R.id.tv_alarm_ring_tips);
		mRingTipsImageView = (ImageView) alarmView
				.findViewById(R.id.img_alarm_ring_tips);
		...
		if (!TextUtils.isEmpty(signName) && !signName.equals("不测量")) {
			...
			mRingTipsImageView.startAnimation(AnimationUtils.loadAnimation(
					this, R.anim.move_left_repeat));
			...
			if (signName.toLowerCase().equals("url")) {
				...
				View signView = getLayoutInflater().inflate(
						R.layout.alarm_tab_sign, null);
				mTopMsgView = (TopMsgView) signView.findViewById(R.id.msgview);
				mInputSignView = (InputSignView) signView
						.findViewById(R.id.sign_alarm_tab);
				mInputSignView.showUploadButton(false);
				mInputSignView.setDataList(new SignCore(this)
						.getSignTypes(signName));
				signView.findViewById(R.id.btn_alarm_tab_connet)
						.setOnClickListener(this);
				signView.findViewById(R.id.btn_alarm_tag_upload)
						.setOnClickListener(this);
				views.add(signView);
			}
		}
		...
		TextView tvContent = (TextView) alarmView
				.findViewById(R.id.tv_alarm_content);
		TextView tvTtitle = (TextView) alarmView
				.findViewById(R.id.tv_next_alarm_content);
		TextView tvTime = (TextView) alarmView.findViewById(R.id.tv_alarm_time);
		ImageView imgRingUpImageView = (ImageView) alarmView
				.findViewById(R.id.img_alarm_ring_up);
		AnimationDrawable drawable = (AnimationDrawable) imgRingUpImageView
				.getDrawable();
		drawable.start();
		...
		flAlarmContent = (FrameLayout) alarmView
				.findViewById(R.id.fl_alarm_content);
	...
	private View loadWebView(String url) {
		...
		View view = getLayoutInflater().inflate(R.layout.xr_layout_webviewer,
				null);
		final WebView webView = (WebView) view
				.findViewById(R.id.xr_webviewer_webView);
		final View layoutView = view
				.findViewById(R.id.ll_setting_webview_loadding);
		final ImageView loadView = (ImageView) layoutView
				.findViewById(R.id.img_xr_webview_loadding);
		...
		webView.addJavascriptInterface(new AndroidAlaramRingInterface(),
				"android"); // 添加Android接口
	...
	private void initPlayer() {
		...
		try {
			...
			Toast.makeText(this, "闹钟播放失败！" + e.getMessage(), Toast.LENGTH_SHORT)
					.show();
			e.printStackTrace();
		}
	}
	...
	public void onSignCoreError(int code, String msg) {
		mTopMsgView
				.setBackgroundColor(getResources().getColor(R.color.mihuang));
		mTopMsgView.setMsg(msg);
		mTopMsgView.show();
	}
