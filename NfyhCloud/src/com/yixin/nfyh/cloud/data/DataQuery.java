...
public class DataQuery implements IUser, IDict {
	...
	private List<Dicts> getDictListByCode(String code, String key, String value)
			throws SQLException {
		Where<Dicts, Long> query = this.dicts.queryBuilder().where()
				.eq("code_name", code);
		if (key != null) {
	...
	private void addOrUpdateDictModuleOrSignType(String[] arrs, String uid,
			String dictName) {
		try {
			// 首先删除该用户的数据
			DeleteBuilder<Dicts, Long> deltebuilder = this.dicts
					.deleteBuilder();
			deltebuilder.where().eq("code_name", uid).and()
					.eq("name", dictName);
			deltebuilder.delete();
