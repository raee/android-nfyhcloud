...
public class SignCore implements SoapConnectionCallback<List<UserSigns>>
{
	...
	public void converGuestDataToUser()
	{
		...
		if (datas.size() > 0)
		{
			new RuiDialog.Builder(mContext).buildLeftButton("不同步", new DialogInterface.OnClickListener()
			{
	 ...
	 * @throws SQLException
	 */
	public List<SignTypes> converSignDataModelToDbModel(List<SignDataModel> models)
	{
		...
		try
		{
			for (SignDataModel m : models)
			{
				...
				// 如果是心电数据则先保存
				if (typeName.equals("心电"))
				{
					List<SignTypes> list = new ArrayList<SignTypes>();
					list.add(type);
					saveUserSign(list);
					Log.i("rae", "先保存心电数据：" + type.getDefaultValue());
					type.setDefaultValue("点击查看");
				}
	...
	{
		if (m.getIsSign() != 1) { return null; } // 不是体征，TODO：综合评价，根据不同的体征组合计算。
