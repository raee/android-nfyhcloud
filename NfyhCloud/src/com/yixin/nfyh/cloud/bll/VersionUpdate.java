 ...
 * 
 */
public class VersionUpdate implements
		SoapConnectionCallback<VersionUpdateModel> {
	...
	public VersionUpdate(Context context) {
		...
		try {
			mCurrentVersionCode = mContext.getPackageManager().getPackageInfo(
					mContext.getPackageName(), 0).versionCode;
		} catch (NameNotFoundException e) {
			e.printStackTrace();
		}
		int version = mContext.getSharedPreferences("VersionUpdate",
				Context.MODE_PRIVATE).getInt("Version", 0);
	...
	public void check() {
		...
		VersionUpdateServer versionApi = NfyhWebserviceFactory.getFactory(
				mContext).getVersionUpdateServer();
	...
	public void onSoapConnectSuccess(final VersionUpdateModel data) {
		...
		String msg = Html.fromHtml(
				"<p>版本号：" + data.getVersionName() + "，更新内容：</p>"
						+ data.getUpdateContent()).toString();
		...
		ruiDialog.setRightButton("更新", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				client.get(data.getDownloadUrl(),
						new FileAsyncHttpResponseHandler(mContext) {
							...
							public void onSuccess(int arg0, Header[] arg1,
								...
								try {
									if (Environment.getExternalStorageState()
										...
										File newPath = new File(mDownloadPath
												+ data.getVersionCode()
												+ ".apk");
										...
										intent.setDataAndType(
												Uri.fromFile(newPath),
												"application/vnd.android.package-archive");
										Notification notification = new Notification();
										notification.icon = mContext
							...
							public void onProgress(int bytesWritten,
								...
								String message = "正在下载新版本，共" + totalSize / 1024
										/ 1024 + "MB（" + progress + "%)";
								ruiDialog.setMessage(Html.fromHtml(message)
										.toString());
