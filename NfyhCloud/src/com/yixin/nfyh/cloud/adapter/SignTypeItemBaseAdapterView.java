...
public abstract class SignTypeItemBaseAdapterView extends BaseAdapter {
	...
	public SignTypeItemBaseAdapterView(Context context) {
		...
		datas = new ArrayList<SignTypes>();
		app = (NfyhApplication) context.getApplicationContext();
	...
	public void loadLastValues() {
		try {
			...
			for (SignTypes itemSignTypes : datas) {
				UserSigns usersign = apiSign.getLastUserSignsByType(
						user.getUid(), itemSignTypes); // 获取最近一次的数据
				if (usersign != null) {
					itemSignTypes.setDefaultValue(usersign.getSignValue());
					// Log.i("SignTypeItemBaseAdapterView", "赋上次体征值：" +
					// itemSignTypes.getName() + "|" + usersign.getSignValue());
				}
