...
public class SignDetailActivity extends BaseActivity implements SignCoreListener
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		{
			boolean isAutoUpload = mConfig.getBooleanConfig(ConfigServer.KEY_AUTO_UPLOAD);
			if (isAutoUpload)
			{
				this.sync();
			}
	...
	private void initSign()
	{
		...
		if (type.equals("-1"))
		{
			...
			mInputSignView.showTipsWindow(new DialogInterface.OnDismissListener()
			{
	...
	public void onSignCoreSuccess(int code, String msg)
	{
		...
		if (code == 200)
		{
			...
			if (!mConfig.getBooleanConfig(ConfigServer.KEY_AUTO_TIPS) || mIsTipsWindowDismiss)
			{ // 没有体征提示的直接显示上传成功。
	...
	{
		Intent intent = new Intent(SignDetailActivity.this, DeviceConnectActivity.class);
		intent.putExtra(DeviceConnectActivity.INTENT_EXTRA_TIPS, msg);
		intent.putExtra(DeviceConnectActivity.INTENT_EXTRA_MESSAGE, "");
		intent.putExtra(DeviceConnectActivity.EXTRA_NAME, DeviceConnectActivity.EXTRA_SHOW_SUCCESS);
	...
	private void sync()
	{
		...
		List<SignTypes> datas = mInputSignView.getDataList();
		
		// 去掉心电数据，因为心电数据是先保存的。
		SignTypes remoteItem;
		for (int i = 0; i < datas.size(); i++)
		{
			if (datas.get(i).getName().equals("心电"))
			{
				remoteItem = datas.get(i);
				datas.remove(i); // 不同步心电数据。				
				mSignCore.saveUserSign(datas);
				// 保存后重新赋值
