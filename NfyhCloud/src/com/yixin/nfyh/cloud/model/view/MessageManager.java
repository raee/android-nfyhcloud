...
public final class MessageManager
{
	...
	public static Intent getIntent(Context context, Messages item)
	{
		...
		if (TextUtils.isEmpty(item.getIntentName())) { return intent; }
		...
		if (!TextUtils.isEmpty(item.getTypeCode()))
		{ // 类型
	...
	private static void setViewMessageIntent(Context context, Messages item, Intent intent)
	{
	...
	// 创建闹钟
	public static void createAlarm(Context context, Messages item, String entityJson)
	{
		// 闹钟处理
		if (MESSAGE_TYPE_ALARM.equals(item.getTypeCode()) && !TextUtils.isEmpty(entityJson))
		{
			// 保存闹钟
	...
	{
		if (TextUtils.isEmpty(model.getIntentDate())) { return false; }
	...
	public static Bundle getExtras(String extras)
	{
		...
		if ("null".equals(extras) || extras == null) { return result; }
		try
		{
