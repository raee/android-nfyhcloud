...
public class UploadUtil
{
	...
	public void uploadFile(String filePath, String fileKey, String RequestURL,
			Map<String, String> param, Object data)
	{
		if (filePath == null)
		{
			sendMessage(UPLOAD_FILE_NOT_EXISTS_CODE, "文件不存在", data);
			return;
		}
		try
		{
			File file = new File(filePath);
			uploadFile(file, fileKey, RequestURL, param, data);
		}
		catch (Exception e)
		{
			sendMessage(UPLOAD_FILE_NOT_EXISTS_CODE, "文件不存在", data);
			...
			final Object data)
	{
		...
		new Thread(new Runnable()
		{
			...
			public void run()
			{
				toUploadFile(file, fileKey, RequestURL, param, data);
			}
		}).start();
			...
			Map<String, String> param, Object data)
	{
		...
		try
		{
			...
			while ((len = is.read(bytes)) != -1)
			{
				...
				if (onUploadProcessListener != null)
					onUploadProcessListener.onUploadProcess(curLen, data);
			}
			is.close();
			...
			if (res == 200)
			{
				...
				sendMessage(UPLOAD_SUCCESS_CODE, "上传结果：" + result, data);
				return;
			}
			else
			{
				Log.e(TAG, "request error");
				sendMessage(UPLOAD_SERVER_ERROR_CODE, "上传失败：code=" + res, data);
				return;
			}
		}
	...
	public static interface OnUploadProcessListener
	{
		 ...
		 * @param message
		 */
		void onUploadDone(int responseCode, String message, Object data);
		 ...
		 * @param uploadSize
		 */
		void onUploadProcess(int uploadSize, Object data);
