...
{ // 操作符号定义
	...
	public RangeCompareable(Context context) throws SQLException
	{
		...
		try
		{
			...
			for (SignRange range : ranges)
			{
				...
				for (SignTips tip : tips)
				{
					...
					boolean compareResult = compareWithSymbol(symbol, left,
							right, value, taget, result); // 比较范围的结果是否满足条件，如果是则把结果添加到集合中
	 ...
	 * @return
	 */
	private boolean compareWithSymbol(int symbol, double left, double right,
			double src, double taget, SignTipsViewModel m)
	{
